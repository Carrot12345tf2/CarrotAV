...
int mbedtls_platform_win32_vsnprintf(char *s, size_t n, const char *fmt, va_list arg)
{
    ...
    /* Windows XP's msvcrt.dll does NOT export vsnprintf_s (it was added in
     * later CRTs), so calling it makes the whole exe fail to load with an
     * "Entry Point Not Found" error before main() ever runs. Force the plain
     * vsnprintf path, which XP has had since forever. */
#if 0
