...
/* XP-compatible entropy: the legacy CryptoAPI RNG. bcrypt.dll does not exist
 * on Windows XP, so BCryptGenRandom is unavailable; CryptGenRandom has been
 * present since Windows 2000 and is a fine CSPRNG source for seeding. */
#include <windows.h>
#include <wincrypt.h>
                                  ...
                                  size_t *olen)
{
    ...
    *olen = 0;
    ...
    if (!CryptGenRandom(prov, (DWORD) len, output)) {
